      ...
      if (roleValue) {
        ...
        if (!isNaN(roleNumber) && roleNumber >= 1 && roleNumber <= 3) {
          ...
          if (typeof roleValue === "string") {
            if (roleValue.toLowerCase().includes("fornecedor"))
              userType = UserType.FORNECEDOR;
            else if (roleValue.toLowerCase().includes("administrador"))
              userType = UserType.ADMINISTRADOR;
            else if (roleValue.toLowerCase().includes("consumidor"))
              userType = UserType.CONSUMIDOR;
          }
        }
      }
